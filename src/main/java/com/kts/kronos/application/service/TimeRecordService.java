...
import static com.kts.kronos.domain.model.enuns.StatusRecord.IMPORTED;
...
public class TimeRecordService implements TimeRecordUseCase {
    ...
    public ActionResponse registerTimeForEmployee(UUID employeeId, GeolocationRequest request) {
        ...
        try {
            ActionResponse response = tracing().observe("kronos.time_record.checkin", () -> {
                ...
                return doCheckinCore(employee, request, implicitBreakCreated, convertedFromStatus);
            });
    ...
    public ActionResponse registerTimeFromTerminal(UUID targetEmployeeId, GeolocationRequest request) {
        long startedAt = System.nanoTime();
        final StatusRecord[] convertedFromStatus = new StatusRecord[1];
        final boolean[] implicitBreakCreated = {false};
        ...
        try {
            ...
            if (CHECKOUT.equals(response.actionType())) {
                metrics().timeRecordCheckoutSuccess();
                metrics().recordTimeRecordDuration("checkout", Duration.ofNanos(System.nanoTime() - startedAt));
                log.info("event=time_record_register result=success action=checkout source=terminal");
            } else {
                metrics().timeRecordCheckinSuccess();
                metrics().recordTimeRecordDuration("checkin", Duration.ofNanos(System.nanoTime() - startedAt));
                if (implicitBreakCreated[0]) metrics().timeRecordImplicitBreak();
                if (convertedFromStatus[0] == StatusRecord.DAY_OFF) metrics().timeRecordDayOffConverted();
                if (convertedFromStatus[0] == StatusRecord.ABSENCE) metrics().timeRecordAbsenceConverted();
                log.info("event=time_record_register result=success action=checkin source=terminal");
            }
            ...
            String reason = resolveTimeRecordFailureReason(e);
            metrics().timeRecordFailure(reason);
            log.warn("event=time_record_register result=failure reason={} source=terminal", reason);
            throw e;
        }
    }

    private ActionResponse doCheckinCore(
            Employee employee,
            GeolocationRequest request,
            boolean[] implicitBreakCreated,
            StatusRecord[] convertedFromStatus
    ) {
    ...
    public List<TimeRecordResponse> listReport(UUID employeeId, ListReportRequest req) {
        ...
        var allPossibleReportStatuses = EnumSet.of(
                ...
                WORK_TIME_REQUEST, WORK_TIME_REJECTED, IMPORTED
        );
    ...
    public List<Long> requestVacation(RequestVacationRequest request) {
        ...
        for (int i = 0; i < daysBetween; i++) {
            ...
            var savedVacation = recordRepository.save(vacationRequestRecord);
            createdRecordIds.add(savedVacation.timeRecordId());
    ...
    private User getManagerApprover(UUID managerUserId, UUID companyId, String invalidRoleMessage, boolean forbiddenWhenInvalidRole) {
        ...
        // Verifica se o gestor pertence à empresa via tb_employee (caso direto)
        // ou via UCA (gestores que gerenciam múltiplas empresas)
        boolean belongsDirectly = employeeProvider.findById(managerUser.employeeId())
                .map(e -> e.companyId().equals(companyId))
                .orElse(false);

        if (!belongsDirectly) {
            boolean hasUca = userCompanyAccessProvider.existsActiveByUserIdAndCompanyId(managerUserId, companyId);
            if (!hasUca) {
