    ...
    public LivenessVerificationResult verify(String faceImageBase64, LivenessOperation operation, UUID employeeId) {
        try {
            if (faceImageBase64 == null || faceImageBase64.isBlank()) {
                log.warn("event=liveness_verification_failed reason=empty_image operation={} employeeId={}",
            ...
            if (faceImageBase64.length() < MIN_BASE64_LENGTH) {
                log.warn("event=liveness_verification_failed reason=image_too_small operation={} employeeId={}",
                        operation, employeeId);
                return LivenessVerificationResult.failed(PROVIDER_NAME, "IMAGE_TOO_SMALL");
            }

            if (faceImageBase64.length() > MAX_BASE64_LENGTH) {
                log.warn("event=liveness_verification_failed reason=image_too_large operation={} employeeId={}",
                        operation, employeeId);
            ...
            try {
                ...
                log.warn("event=liveness_verification_failed reason=invalid_base64 operation={} employeeId={}",
                        operation, employeeId);
                return LivenessVerificationResult.failed(PROVIDER_NAME, "INVALID_BASE64");
            }

            log.info("event=liveness_verification_passed operation={} employeeId={} provider={}",
                    operation, employeeId, PROVIDER_NAME);

            // Retornar "passou" com confiança baixa (não é real liveness)
            return LivenessVerificationResult.passed(PROVIDER_NAME, 0.3);

        } catch (Exception e) {
            log.error("event=liveness_verification_error operation={} employeeId={} error={}",
                    operation, employeeId, e.getMessage(), e);
